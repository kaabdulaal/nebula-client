  ...
  Future<void> _startNebula() async {
    ...
    try {
      ...
      if (_isCustom) {
        ...
        // Standard Nebula: Try to sync from Gist
        final success = await repo.syncCredentials();
        if (success) {
          creds = await repo.getCredentials();
        } else {
          // Fallback to factory keys in repo.getCredentials() if Gist fails
          creds = await repo.getCredentials();
        }
      }
      ...
      if (creds != null) {
        ...
        // Move to Auth/Master Pass screen
        if (mounted) {
          context.go('/home');
        }
      } else {
